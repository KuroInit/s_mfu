...
| `HF_CACHE_DIR` | `~/.cache/huggingface` | Host path mounted into the container as `/hf_cache` (set before `docker compose run`) |
| `HF_CACHE` | `$HF_HOME` or `~/.cache/huggingface` | Model weight root inside the container (override for `sweep.sh` only) |
